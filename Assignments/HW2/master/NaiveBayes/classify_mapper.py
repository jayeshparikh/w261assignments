...
for line in sys.stdin:
    ...
    conditionals = np.array([MODEL.get(word,(0,0)) for word in words])      # <--- SOLUTION --->
    logpHam, logpSpam = conditionals.sum(axis=0) + MODEL['ClassPriors']     # <--- SOLUTION --->
    pred_class = np.argmax([logpHam, logpSpam])                             # <--- SOLUTION --->
